#!/usr/bin/env python3
"""
This script is used to selectively comment "not ok" lines in TAP files
so the openQA TAP parser ignores them
"""
...
def flush(file: TextIO, lines: List[str], comment_block: bool = False) -> None:
    """
...
def comment_not_ok(path: str, tests: List[str]) -> None:
    ...
    skipped2 = (
        re.compile(rf"not ok \d+ \[(?:{'|'.join(numeric)})\]") if numeric else None
    )
...
if __name__ == "__main__":
    if len(sys.argv) > 2:
        ...
        sys.exit(f"Usage: {sys.argv[0]} FILE TEST...")
